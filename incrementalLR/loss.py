...
class MCRGANloss(nn.Module):
    ...
    def __init__(self, gam1=1., gam2=1., eps=0.5, numclasses=1000, mode=1, lambd=15, class_step=1):
        super(MCRGANloss, self).__init__()

        self.criterion = MCRGANloss1(eps=eps)
        self.num_class = numclasses
        self.class_step = class_step
        self.train_mode = mode
        self.lambd = lambd

    def forward(self, Z, Z_bar, real_label, train_mode=1):
        ...
        if train_mode == 1:
            ...
            errD = loss_z + loss_h
            empi = [loss_z, loss_h]
            true_label = max(real_label)

            for i in range(true_label - self.class_step + 1, true_label + 1):
                ...
                errD += self.lambd * loss
        elif train_mode == 0:
            new_Z = torch.cat((Z, Z_bar), 0)
            new_label = torch.cat((torch.zeros_like(real_label), torch.ones_like(real_label)))
            errD, empi, _ = self.criterion(new_Z, new_label)
        elif train_mode == 2:
            errD, empi, _ = self.criterion(Z, real_label)


        else:
            raise ValueError()

        return errD, loss_z, loss_h, loss


class MCRGANlossDISC(nn.Module):

    def __init__(self, gam1=1., gam2=1., eps=0.5, numclasses=1000, mode=1, class_step=1):
        super(MCRGANlossDISC, self).__init__()
    ...
    def forward(self, Z, Z_bar, real_label, train_mode=1):
        ...
        if train_mode == 1:
            ...
            loss_a, _, _ = self.criterion(Z, torch.cat(
                (torch.zeros_like(real_label[real_label == true_label]),
                 torch.ones_like(real_label[real_label != true_label]))
            ))

            errD = loss_z + loss_h + loss_a

            for i in range(true_label - self.class_step + 1, self.class_step + 1):
                new_Z = torch.cat((Z[real_label == i], Z_bar[real_label == i]), 0)
                new_label = torch.cat(
                    (torch.zeros_like(real_label[real_label == i]),
                     torch.ones_like(real_label[real_label == i]))
        ...
        return errD, loss_z, loss_h, 0


class MCRGANlossGEN(nn.Module):

    def __init__(self, gam1=1., gam2=1., eps=0.5, numclasses=1000, mode=1):
        super(MCRGANlossGEN, self).__init__()
    ...
    def forward(self, Z, Z_bar, real_label, train_mode=1):
        ...
        return errD, loss_z, loss_h, loss, third_loss



class MCRGANloss1(nn.Module):

    def __init__(self, gam1=1., gam2=1., gam3=1., eps=0.5, numclasses=1000, mode=1):
        super(MCRGANloss1, self).__init__()
    ...
    def compute_compress_loss(self, Z, Pi):
        ...
        for j in range(Pi.shape[1]):
            ...
            log_det = 1. if Pi[:, j].sum() == 0 else torch.logdet(I + scalar * Z_ @ Z_.T)
